<template>
  <div class="admin-login-wrap" v-if="!authed">
    <form class="form card admin-login-card" @submit.prevent="login">
      <h1 class="serif" style="text-align:center;margin:0">RIKAI 後台</h1>
  ...
  <div class="admin" v-else>
    <aside class="aside">
      ...
      <router-link to="/admin/news">最新訊息</router-link>
      <router-link to="/admin/banners">首頁輪播</router-link>
      <router-link to="/admin/stores">門市據點</router-link>
...
const email = ref('admin@rikai.tw')
const password = ref('rikai888')
const err = ref('')
